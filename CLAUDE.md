...
Greenfield repo (stack not yet chosen — record it here on first commit). Telemetry for AI-agent token usage: collection, storage, reporting. Shell: plain zsh on macOS, no env preamble required yet. No dev stack/ports yet — record the command + ports here when one exists. Long-form docs live in `docs/`. Tracker: JIRA, project **AgentOS** (key AOS) at example.atlassian.net — modern scrum template (Epic/Story/Task/Bug/Idea); current work: epic AOS-1. (Migrated from Linear 2026-08-03.)
