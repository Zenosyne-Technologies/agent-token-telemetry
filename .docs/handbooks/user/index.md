...
| [[enabling-remote-telemetry]] | `commands/enable-remote.md`, `commands/enable.md`, `scripts/remote_migrate.py`, `scripts/settings.py`, `scripts/supabase_backend.py`, `scripts/dashboard.py`, `scripts/dashboard.html`, `scripts/backfill_summary.py` | Turning on the shared remote (Supabase) backend with `/enable-remote` — the local-vs-remote choice, the URL + publishable-key env-var NAME setup (the key value never entered in chat), applying the schema and running the RLS live-verification, login, ensuring the remote users row, migrate-or-start-fresh, the reversible switch, the dashboard's local-only banner once remote is active, and its own-price warning banner with the "backfill available" line (never applies anything; hidden when stale and on remote) | active | 2026-09-23 |
