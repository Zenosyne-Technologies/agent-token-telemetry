...
| [[enabling-remote-telemetry]] | `commands/enable-remote.md`, `commands/enable.md`, `scripts/remote_migrate.py`, `scripts/settings.py`, `scripts/supabase_backend.py` | Turning on the shared remote (Supabase) backend with `/enable-remote` — the local-vs-remote choice, the URL + publishable-key env-var NAME setup (the key value never entered in chat), applying the schema and running the RLS live-verification, login, ensuring the remote users row, migrate-or-start-fresh, and the reversible switch | active | 2026-09-22 |
