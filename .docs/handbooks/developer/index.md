...
| [[remote-read-parity]] | `supabase/reports.sql`, `scripts/supabase_backend.py`, `scripts/report.py`, `scripts/storage.py`, `tests/test_report_parity.py` | Reading reports from the remote backend with numbers identical to local SQLite — the `SECURITY INVOKER` `reports.sql` view/RPC (RLS-respecting, own-rows-only), the client `read_for_report` mapping and `report.py` routing on `active_backend`, and the dual-dialect drift risk guarded by the cross-dialect golden test (SQLite reference in CI, Postgres-gated equivalence, maintainer live step). | active | 2026-09-22 |
